...
SIM_WIDTH = 350.0  # Increased for more maneuver room
SIM_HEIGHT = 350.0
TIME_STEP = 0.1  # s
MAX_SIM_TIME = 80.0  # s
...
RUNWAY_START_XY = np.array([BASE_CENTER_XY[0] - 50.0, SIM_HEIGHT / 2])  # Longer runway approach area
RUNWAY_END_XY = np.array([BASE_CENTER_XY[0] + 50.0, SIM_HEIGHT / 2])  # 100m runway
RUNWAY_WIDTH = 10.0
...
NUM_UAVS = 1
LEADER_ID = 0
UAV_MAX_SPEED_XY = 22.0  # Slightly reduced for testing turns
UAV_MIN_SPEED_XY = 8.0
UAV_ACCELERATION_XY = 2.5
UAV_DECELERATION_XY = 2.5
UAV_MAX_VERTICAL_SPEED = 3.5
UAV_MAX_TURN_RATE = math.radians(18.0)  # Max turn rate in radians/s (Reduced)
UAV_TAKEOFF_SPEED = 16.0

UAV_RADIUS_VIS = 2.0
UAV_CRUISE_ALTITUDE = 60.0  # Increased cruise altitude
UAV_POSITIONING_TOLERANCE_XY = 4.0  # Slightly larger tolerance for reaching waypoints
UAV_ALTITUDE_TOLERANCE = 2.0
UAV_HEADING_TOLERANCE = math.radians(15.0)  # Looser for "aligned" check during transitions
UAV_PATH_HISTORY_LENGTH = 250

KP_HEADING = 0.7  # Gain for correcting heading (Further Reduced)
KP_ALT_FLIGHT = 0.9
...
class UAVStatus(Enum):
    ...
    CRUISING = 3
    WAYPOINT_REACHED_TRANSITION = 4  # Brief state to handle waypoint switch


# --- Helper Functions --- (normalize_angle, angle_diff remain the same)
...
class UAV:
    ...
    def _update_flight_dynamics(self, desired_heading, desired_speed, desired_altitude, time_step_arg):
        ...
        # Limit turn rate at lower speeds slightly more gracefully
        if self.current_speed_xy < UAV_MIN_SPEED_XY and self.status != UAVStatus.ACCELERATING_ON_RUNWAY:
            ...
            effective_max_turn_rate *= max(0.2, speed_ratio ** 1.5)  # Allow some turn even at min_speed
        ...
        if can_change_alt and abs(alt_error) > UAV_ALTITUDE_TOLERANCE:
            ...
            self.vertical_speed = np.clip(-UAV_MAX_VERTICAL_SPEED * 0.2, -UAV_MAX_VERTICAL_SPEED,
                                          UAV_MAX_VERTICAL_SPEED)  # Slower forced descent
        else:
            self.vertical_speed = 0.0
    ...
    def update(self, time_step_arg):
        d_hdg = self.heading  # Default to current heading if no other target
        d_spd = self.target_speed_xy
        d_alt = self.current_target_altitude

        # Always try to head towards the current waypoint if one exists and UAV is airborne
        if self.current_waypoint_xy is not None and self.status.value >= UAVStatus.CLIMBING_AFTER_TAKEOFF.value:
            # Only calculate new heading if not already very close (to prevent jitter when at waypoint)
            # The actual "stopping" or "waypoint reached" logic is in update_simulation
            vec_to_wp = self.current_waypoint_xy - self.position_xy
            if np.linalg.norm(
                    vec_to_wp) > UAV_POSITIONING_TOLERANCE_XY * 0.1:  # If further than a small fraction of tolerance
        ...
        if self.status == UAVStatus.ACCELERATING_ON_RUNWAY:
            ...
            # d_hdg already set by waypoint logic above (towards climbout_wp)
            d_spd = UAV_MAX_SPEED_XY * 0.85  # Climb speed
            d_alt = self.current_target_altitude  # Should be UAV_CRUISE_ALTITUDE
        elif self.status == UAVStatus.CRUISING:
            # d_hdg already set by waypoint logic
            d_spd = UAV_MAX_SPEED_XY * 0.8
            d_alt = self.current_target_altitude  # Should be UAV_CRUISE_ALTITUDE
        elif self.status == UAVStatus.IDLE_ON_GROUND:
            d_spd = 0.0
            d_alt = BASE_ALTITUDE
            # d_hdg remains current heading
...
uavs = []
simulation_time = 0.0
# Adjusted Waypoint distances and angles
climb_out_distance = 80.0
wp1_distance_after_climbout = 120.0
wp2_offset_distance = 100.0
wp2_turn_angle_from_wp1_heading = math.radians(60)  # 60 degree left turn relative to path to WP1
...
def initialize_simulation():
    ...
    # Calculate heading from climbout_wp to wp1
    vec_co_to_wp1 = test_waypoint_1 - test_waypoint_climbout
    heading_co_to_wp1 = math.atan2(vec_co_to_wp1[1], vec_co_to_wp1[0])

    # WP2 is relative to WP1, with a turn from the heading towards WP1
    wp2_heading = normalize_angle(heading_co_to_wp1 + wp2_turn_angle_from_wp1_heading)  # Apply turn
...
# --- Matplotlib Visualization Setup --- (largely the same)
...
wp_climbout_artist, = ax.plot([], [], 'ms', markersize=7, label="WP ClimbOut")  # Changed marker
wp1_artist, = ax.plot([], [], 'gs', markersize=7, label="Waypoint 1")  # Changed marker
wp2_artist, = ax.plot([], [], 'bs', markersize=7, label="Waypoint 2")  # Changed marker
...
def update_simulation(frame_num):
    ...
    current_wp_for_uav = uav.current_waypoint_xy  # For checking proximity
    ...
    if uav.status == UAVStatus.IDLE_ON_GROUND:
        ...
        if current_wp_for_uav is None:
            ...
            # anim.event_source.stop() # Stop if no waypoint
        elif np.linalg.norm(uav.position_xy - current_wp_for_uav) < UAV_POSITIONING_TOLERANCE_XY:
            uav.status = UAVStatus.WAYPOINT_REACHED_TRANSITION  # Enter transition state
            print(f"T+{simulation_time:.1f}s: UAV {uav.id} Reached WP: {current_wp_for_uav}. Transitioning.")
            # Logic to switch to next waypoint will be in WAYPOINT_REACHED_TRANSITION

    elif uav.status == UAVStatus.WAYPOINT_REACHED_TRANSITION:
        # This state is brief, just to set the next waypoint and revert to CRUISING
        if np.array_equal(current_wp_for_uav, test_waypoint_1):
            ...
            uav.target_speed_xy = UAV_MIN_SPEED_XY
            uav.status = UAVStatus.CRUISING  # Or a new "Holding" status
            if anim.event_source: anim.event_source.stop()
        else:  # Arrived at climbout or unknown
            uav.current_waypoint_xy = test_waypoint_1  # Default to WP1 after climbout
            uav.status = UAVStatus.CRUISING
            print(
    ...
    line_len = uav.current_speed_xy * 0.2 + 5  # Adjusted for visibility
